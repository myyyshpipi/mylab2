package org.example.Model.Orc;

import static org.example.Model.Orc.OrcRace.*;

/**
 * "Concrete builder"
 */
public class MistyMountsOrcBuilder extends OrcBuilder {

    public MistyMountsOrcBuilder() {
        super();
        this.race = MISTYMOUNTS_ORC;
    }

    @Override
    public OrcBuilder buildPower() {
        int min = 1;
        int max = 50;
        int power = min + (int) (Math.random() * (max - min));
        this.power = (int) (1.30 * power);
        return this;
    }

    @Override
    public OrcBuilder buildAgility() {
        int min = 1;
        int max = 100;
        int audacity = min + (int) (Math.random() * (max - min));
        this.agility = (int) (1.30 * audacity);
        return this;
    }

    @Override
    public OrcBuilder buildIntelligence() {
        int min = 1;
        int max = 50;
        int intelligence = min + (int) (Math.random() * (max - min));
        this.intelligence = (int) (0.30 * intelligence);
        return this;
    }

    @Override
    public OrcBuilder buildHealth() {
        int min = 50;
        int max = 200;
        this.health = min + (int) (Math.random() * (max - min));
        return this;
    }

}

